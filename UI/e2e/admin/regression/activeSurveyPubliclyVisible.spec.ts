...
test.beforeEach(async ({ page, Pom }) => {
  await page.goto('');
  // the survey id is visible in list mode, not tile mode. If the app is on tile mode, this just clicks the list mode for the test to pass
  await page.locator('#survey-search i').nth(4).click({ timeout: 30000 });
});
...
test.describe('Activate Survey Scenario(s)', () => {
  ...
  test('activated survey is visible on publication index page', async ({ page, Pom }) => {
    ...
    const publicSurveyIndexPageOn = page.locator('#listpublic').getByText('On').first();
